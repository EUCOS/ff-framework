...
string Make_Output_File_Name(string Base_Output_File_Name, 
			     ...
			     Simply_Paired_Layers, double Cutoff_Threshold);
...
int main(int argc, char* argv[])
{
  ...
  for(int a=2; a<argc; ++a)
    {
      if(Extension_Orders.at(a-2)<10&&argv[a][1] == 'n')
	Simply_Paired_Layers.push_back(false);
      else if(Extension_Orders.at(a-2)>=10&&argv[a][2] == 'n')
	Simply_Paired_Layers.push_back(false);
      else
  ...
  Systematic_Builder Bob_The_Builder(Initial_Conditions, Large_ST_Dimensions,
				     ...
				     Simply_Paired_Layers);
  Bob_The_Builder.Perform_Approximate_Search(Cutoff_Threshold);
			     ...
			     Simply_Paired_Layers, double Cutoff_Threshold)
{
  ...
  ss<<"_A"<<int(100*Cutoff_Threshold);
  ss<<".txt";
  return ss.str();
