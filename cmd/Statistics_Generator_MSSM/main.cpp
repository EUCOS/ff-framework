/*
	...
	containing at least one MSSM group. 
...
int Count_Barred_Generations(const LEEFT& LEEFT_Input,
		int SU3_Index, int SU2_Index);
map<int, int> Find_Barred_Generations
	(const set<LEEFT>& Unique_LEEFTs);
...
int main()
{
	...
	string Output_File_Name = "./tex/MSSM_Model_Statistics.tex";

	//Set the Latex document title.
	string Doc_Title = 
		"Statistical Report for NAHE Based MSSM Models, ";
	...
	cout<<"Reading unique LEEFTs containing MSSM gauge groups."<<endl;
	...
	map<int, int> Barred_Generations = 
		Find_Barred_Generations(Unique_LEEFTs);
	//NA Singlets.
	...
	//Chiral matter generations.
	Latex_Out<<"\\textbf{(3,2) generations.}"<<endl;
	Latex_Out<<"\\vspace{3 mm}\\\\"<<endl;
	...
	Latex_Out<<"\\textbf{(3,1) generations.}"<<endl;
	Latex_Out<<"\\vspace{3 mm}\\\\"<<endl;
	Jared_Greenwald.Write_Matter_Generations(Latex_Out,
			Barred_Generations);
	Latex_Out<<"\\vspace{3 mm}"<<endl;
...
bool Has_MSSM_Group(const LEEFT& LEEFT_Input)
{
	...
	//Look for the U(1) first, it's the easiest to spot.
	...
	for(int a=0; a<LEEFT_Input.Gauge_Groups().size(); ++a)
	{
		...
		if(!Has_SU2 && (LEEFT_Input.Gauge_Groups().at(a).Class() == 'A')
		...
		int SU2_Index)
{
	...
	for(int a=0; a<LEEFT_Input.Matter_Representations().size(); ++a)
	{
		if(!Has_Hidden_Sector_Charge(LEEFT_Input.Matter_Representations().
					at(a).Rep_Dimension(), SU3_Index, SU2_Index))
		{
			if((LEEFT_Input.Matter_Representations().at(a).
						Rep_Dimension().at(SU3_Index).Dimension() == -3) && 
					(LEEFT_Input.Matter_Representations().at(a).
					 Rep_Dimension().at(SU2_Index).Dimension() == 2))
			{
			...
			}else if((LEEFT_Input.Matter_Representations().at(a).Rep_Dimension().at(SU3_Index).Dimension() == 3) &&
					(LEEFT_Input.Matter_Representations().at(a).Rep_Dimension().at(SU2_Index).Dimension() == 2))
			{
...
int Count_Barred_Generations(const LEEFT& LEEFT_Input, 
		int SU3_Index, int SU2_Index)
{
	//Bad naming convention, I know. Different kind of bar here.
	//We're switching the SU(2) doublet for the Barred_Generation.
	int Unbarred_Generations = 0;
	int Barred_Generations = 0;
	for(int a=0; a<LEEFT_Input.Matter_Representations().size(); ++a)
	{
					...
					at(a).Rep_Dimension(), SU3_Index, SU2_Index))
		{
			if((LEEFT_Input.Matter_Representations().at(a).Rep_Dimension().
					...
					(LEEFT_Input.Matter_Representations().at(a).Rep_Dimension().
					 at(SU2_Index).Dimension() == 1))
			{
				Barred_Generations += LEEFT_Input.Matter_Representations().
					at(a).Duplicates();
			}else if((LEEFT_Input.Matter_Representations().at(a).Rep_Dimension().at(SU3_Index).Dimension() == 3) && 
					(LEEFT_Input.Matter_Representations().at(a).Rep_Dimension().at(SU2_Index).Dimension() == 1))
			{
				Unbarred_Generations += LEEFT_Input.Matter_Representations().at(a).Duplicates();
			}//Close if/else for counting the barred and unbarred 
			//representations.
		}//Close if statement on hidden sector charge.
	}//Close for loop on Representations.
	return abs(Unbarred_Generations - Barred_Generations);
}//Close Count_Barred_Generations.

map<int, int> Find_Barred_Generations(const set<LEEFT>& Unique_LEEFTs)
{
	map<int, int> Barred_Generations;
			...
			++itUnique_LEEFTs)
	{
		...
		for(int a=0; a<itUnique_LEEFTs->Gauge_Groups().size(); ++a)
		{
			if((itUnique_LEEFTs->Gauge_Groups().at(a).Class() == 'A') && 
					(itUnique_LEEFTs->Gauge_Groups().at(a).Rank() == 2))
				SU3_Indices.push_back(a);
		}//Close for loop finding the SU(3) indices.
		...
		for(int a=0; a<itUnique_LEEFTs->Gauge_Groups().size(); ++a)
		{
			if((itUnique_LEEFTs->Gauge_Groups().at(a).Class() == 'A') && 
					(itUnique_LEEFTs->Gauge_Groups().at(a).Rank() == 1))
				SU2_Indices.push_back(a);
		}//Close for loop finding the SU(2) indices.
		...
		for(int a=0; a<SU3_Indices.size(); ++a)
		{
			for(int b=0; b<SU2_Indices.size(); ++b)
			{
				...
				int Number_Generations = Count_Barred_Generations
					(*itUnique_LEEFTs, SU3_Indices.at(a), SU2_Indices.at(b));
				if(Barred_Generations.find(Number_Generations) != 
						Barred_Generations.end())
					Barred_Generations[Number_Generations]++;
				else
					Barred_Generations[Number_Generations] = 1;
			}//Close for loop over first SU(2) indices.
		}//Close outermost for loop over the SU(3) indices.
	}//Close for loop on Unique_LEEFTs.
	return Barred_Generations;
}//Close Find_Barred_Generations.
...
map<int, int> Find_OS_Charged_Exotics(const set<LEEFT>& Unique_LEEFTs)
{
			...
			++itUnique_LEEFTs)
	{
		...
		for(int a=0; a<SU3_Indices.size(); ++a)
		{
			...
			}//Close for loop over the SU(2) indices.
		}//Close for loop over the SU(3) indices.
	}//Close for loop on Unique_LEEFTs.
	return OS_Charged_Exotics;
}//Close Find_OS_Charged_Exotics.

bool Has_Hidden_Sector_Charge(const vector<Group_Representation>& 
		Representations, int SU3_Index, int SU2_Index)
{
