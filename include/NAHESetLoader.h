...
/*!
 * @file include/NAHESetLoader.h
 * @author Timothy Renner <timothy_renner@baylor.edu>
 *
 * Within we provide a class to load the NAHE set.
 */
#pragma once
...
class NAHESetLoader {
  public:
    NAHESetLoader();
    NAHESetLoader(const NAHESetLoader& Old_NAHESetLoader);

    ~NAHESetLoader() {}

    Model Load_NAHE_Set(const Model& Old_Model, bool With_S);
