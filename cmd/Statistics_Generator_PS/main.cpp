/*
	...
	containing at least one Pati-Salam group. Tabulates gauge group factors,
...
bool Has_PS_Group(const LEEFT& LEEFT_Input);

int Count_Generations(const LEEFT& LEEFT_Input, 
		int SU4_Index, int SU2A_Index, int SU2B_Index);
map<int, int> Find_Generations
	(const set<LEEFT>& Unique_LEEFTs);

int Count_Anti_Generations(const LEEFT& LEEFT_Input,
		int SU4_Index, int SU2A_Index, int SU2B_Index);
map<int, int> Find_Anti_Generations
	(const set<LEEFT>& Unique_LEEFTs);
...
int main()
{
	...
	string Output_File_Name = "./tex/PS_Model_Statistics.tex";

	//Set the Latex document title.
	string Doc_Title = 
		"Statistical Report for NAHE Based Pati-Salam Models, ";
	...
	//Input_File_Names.push_back
	//(FF_Data + "NAHE_Extensions/One_Layer/NR_NAHE_NSV_O2_L1_NSP.txt");

	//int Skipped_Lines = 2;//For no layers.
	//int Skipped_Lines = 3; //For the NAHE NR models.
	int Skipped_Lines = 4;//For one layer.

	cout<<"Reading unique LEEFTs containing Pati-Salam gauge groups."<<endl;
	...
	map<int, int> Anti_Generations = 
		Find_Anti_Generations(Unique_LEEFTs);
	//NA Singlets.
	...
	//Chiral matter generations.
	Latex_Out<<"\\textbf{(4,2,1) generations.}"<<endl;
	Latex_Out<<"\\vspace{3 mm}\\\\"<<endl;
	...
	Latex_Out<<"\\textbf{(4,1,1) generations.}"<<endl;
	Latex_Out<<"\\vspace{3 mm}\\\\"<<endl;
	Jared_Greenwald.Write_Matter_Generations(Latex_Out,
			Anti_Generations);
	Latex_Out<<"\\vspace{3 mm}"<<endl;
...
bool Has_PS_Group(const LEEFT& LEEFT_Input)
{
	...
	{
		if(!Has_SU4 && (LEEFT_Input.Gauge_Groups().at(a).Class() == 'A')
				&& (LEEFT_Input.Gauge_Groups().at(a).Rank() == 3))
			Has_SU4 = true;
		...
		if(Has_SU4 && Has_2_SU2)
			return true;
	}
	return false;
}//Close Has_SO10_Group.

int Count_Generations(const LEEFT& LEEFT_Input, int SU4_Index, 
		int SU2A_Index, int SU2B_Index)
{
	...
	for(int a=0; a<LEEFT_Input.Matter_Representations().size(); ++a)
	{
		if(!Has_Hidden_Sector_Charge(LEEFT_Input.Matter_Representations().
					at(a).Rep_Dimension(), SU4_Index, SU2A_Index, SU2B_Index))
		{
			if((LEEFT_Input.Matter_Representations().at(a).Rep_Dimension().
						at(SU4_Index).Dimension() == -4) && 
			...
			}//Close if/else for counting the barred and unbarred i
			//representations.
...
map<int, int> Find_Generations(const set<LEEFT>& Unique_LEEFTs)
{
	...
	for(itUnique_LEEFTs; itUnique_LEEFTs != Unique_LEEFTs.end(); 
			++itUnique_LEEFTs)
	{
		vector<int> SU4_Indices;
		for(int a=0; a<itUnique_LEEFTs->Gauge_Groups().size(); ++a)
		{
			if((itUnique_LEEFTs->Gauge_Groups().at(a).Class() == 'A') && 
					(itUnique_LEEFTs->Gauge_Groups().at(a).Rank() == 3))
				SU4_Indices.push_back(a);
		}//Close for loop for finding the SU(4) indices.
		...
		for(int a=0; a<SU4_Indices.size(); ++a)
		{
				...
				for(int c=0; c<SU2_Indices.size(); ++c)
				{
					if(c != b)
					{
						int Number_Generations = Count_Generations
							(*itUnique_LEEFTs, SU4_Indices.at(a), SU2_Indices.at(b),
							 SU2_Indices.at(c));
...
int Count_Anti_Generations(const LEEFT& LEEFT_Input, 
		int SU4_Index, int SU2A_Index, int SU2B_Index)
{
	int Unbarred_Anti_Generations = 0;
	int Barred_Anti_Generations = 0;
	for(int a=0; a<LEEFT_Input.Matter_Representations().size(); ++a)
	{
		//Check to see if there is hidden sector charge.
		if(!Has_Hidden_Sector_Charge(LEEFT_Input.Matter_Representations().
					at(a).Rep_Dimension(), SU4_Index, SU2A_Index, SU2B_Index))
		{
			if((LEEFT_Input.Matter_Representations().at(a).Rep_Dimension().
					 ...
					 at(SU2B_Index).Dimension() == 1))
			{
				Unbarred_Anti_Generations += 
					...
					(LEEFT_Input.Matter_Representations().at(a).Rep_Dimension().
					 at(SU2B_Index).Dimension() == 1))
			{
				Barred_Anti_Generations += 
					LEEFT_Input.Matter_Representations().at(a).Duplicates();
			}//Close if/else for counting the barred and unbarred 
			//representations.
		}//Close if/else on hidden sector charge.
	}//Close for loop on Representations.
	return abs(Unbarred_Anti_Generations - Barred_Anti_Generations);
}//Close Count_Anti_Generations.

map<int, int> Find_Anti_Generations(const set<LEEFT>& Unique_LEEFTs)
{
	map<int, int> Anti_Generations;
			...
			++itUnique_LEEFTs)
	{
		...
		for(int a=0; a<SU4_Indices.size(); ++a)
		{
			for(int b=0; b<SU2_Indices.size(); ++b)
			{
				for(int c=(b+1); c<SU2_Indices.size(); ++c)
				{
					int Number_Generations = Count_Anti_Generations
						(*itUnique_LEEFTs, SU4_Indices.at(a), SU2_Indices.at(b),
						 SU2_Indices.at(c));
					if(Anti_Generations.find(Number_Generations) !=
							Anti_Generations.end())
						Anti_Generations[Number_Generations]++;
					else
						Anti_Generations[Number_Generations] = 1;
	...
	return Anti_Generations;
}//Close Find_Anti_Generations.

int Count_OS_Charged_Exotics(const LEEFT& LEEFT_Input, int SU4_Index,
		int SU2A_Index, int SU2B_Index)
{
	...
	for(int a=0; a<LEEFT_Input.Matter_Representations().size(); ++a)
	{
		if(((LEEFT_Input.Matter_Representations().at(a).Rep_Dimension().
				...
				(Has_Hidden_Sector_Charge(LEEFT_Input.Matter_Representations().
																	at(a).Rep_Dimension(),
																	SU4_Index, SU2A_Index, SU2B_Index)))
		...
		for(int a=0; a<SU4_Indices.size(); ++a)
		{
			for(int b=0; b<SU2_Indices.size(); ++b)
			{
				for(int c=0; c<SU2_Indices.size(); ++c)
				{
					if(c != b)
					{
						int SU4_Index = SU4_Indices.at(a);
...
bool Has_Hidden_Sector_Charge(const vector<Group_Representation>& 
		Representations, int SU4_Index, int SU2A_Index, int SU2B_Index)
{
	for(int a=0; a<Representations.size(); ++a)
	{
		if((a != SU4_Index) && (a != SU2A_Index) && 
				(a != SU2B_Index) && 
				(Representations.at(a).Dimension() != 1))
			return true;
	}
	return false;
