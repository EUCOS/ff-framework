/*
	...
	containing at least one E6 group. Tabulates gauge group factors,
...
bool Has_E6_Group(const LEEFT& LEEFT_Input);
...
int Count_HS_Uncharged_Generations(const LEEFT& LEEFT_Input, int E6_Index);
...
int main()
{
	...
	string Output_File_Name = "./tex/E6_Model_Statistics.tex";

	//Set the Latex document title.
	string Doc_Title = "Statistical Report for NAHE Based $E_6$ Models " ;
	...
	//int Skipped_Lines = 3;//For the NAHE NR data set.
	int Skipped_Lines = 4;//For one layer.

	cout<<"Reading unique LEEFTs containing E6 gauge groups."<<endl;
...
bool Has_E6_Group(const LEEFT& LEEFT_Input)
{
	...
	{
		if((LEEFT_Input.Gauge_Groups().at(a).Class() == 'E') &&
				(LEEFT_Input.Gauge_Groups().at(a).Rank() == 6))
			return true;
	}
	return false;
}//Close Has_E6_Group.

int Count_HS_Charged_Generations(const LEEFT& LEEFT_Input, int E6_Index)
{
	...
	for(int a=0; a<LEEFT_Input.Matter_Representations().size(); ++a)
	{
		if(LEEFT_Input.Matter_Representations().at(a).
				Rep_Dimension().at(E6_Index).Dimension() == -27)
		{
...
map<int, int> Find_HS_Charged_Generations(const set<LEEFT>& Unique_LEEFTs)
{
			...
			++itUnique_LEEFTs)
	{
		...
		vector<int> E6_Indices;
		for(int a=0; a<itUnique_LEEFTs->Gauge_Groups().size(); ++a)
		{
			if((itUnique_LEEFTs->Gauge_Groups().at(a).Class() == 'E') && 
					(itUnique_LEEFTs->Gauge_Groups().at(a).Rank() == 6))
				E6_Indices.push_back(a);
		}//Close for loop on the gauge groups of *itUnique_LEEFTs.

		//Loop over the E6 groups, count the generations, 
		//and add them to the map.
		for(int a=0; a<E6_Indices.size(); ++a)
		{
			int Number_HS_Charged_Generations = Count_HS_Charged_Generations
				(*itUnique_LEEFTs, E6_Indices.at(a));
		...
		}//Close for loop on the different E6 indices.
...
int Count_HS_Uncharged_Generations(const LEEFT& LEEFT_Input, int E6_Index)
{
	...
	for(int a=0; a<LEEFT_Input.Matter_Representations().size(); ++a)
	{
		...
		if(!Has_Hidden_Sector_Charge(LEEFT_Input.Matter_Representations().
					at(a).Rep_Dimension(), E6_Index))
		{
 			if(LEEFT_Input.Matter_Representations().at(a).
					Rep_Dimension().at(E6_Index).Dimension() == 27)
				Unbarred_Generations += LEEFT_Input.Matter_Representations().
					at(a).Duplicates();
			else if(LEEFT_Input.Matter_Representations().at(a).
					Rep_Dimension().at(E6_Index).Dimension() == -27)
			...
			++itUnique_LEEFTs)
	{
		...
		for(int a=0; a<itUnique_LEEFTs->Gauge_Groups().size(); ++a)
		{
			if((itUnique_LEEFTs->Gauge_Groups().at(a).Class() == 'E') &&
					(itUnique_LEEFTs->Gauge_Groups().at(a).Rank() == 6))
				E6_Indices.push_back(a);
		}//Close for loop for finding all of the E6 indices.

		//Loop over the E6 groups, count the generations, and 
		//add them to the map.
		for(int a=0; a<E6_Indices.size(); ++a)
		{
			int Number_HS_Uncharged_Generations = 
				Count_HS_Uncharged_Generations(*itUnique_LEEFTs, E6_Indices.at(a));
...
int Count_OS_Charged_Exotics(const LEEFT& LEEFT_Input, int E6_Index)
{
	int OS_Charged_Exotics = 0;
	for(int a=0; a<LEEFT_Input.Matter_Representations().size(); ++a)
	{
		if((LEEFT_Input.Matter_Representations().at(a).Rep_Dimension().
					at(E6_Index).Dimension() != 1) &&
...
map<int, int> Find_OS_Charged_Exotics(const set<LEEFT>& Unique_LEEFTs)
{
	...
	for(itUnique_LEEFTs; itUnique_LEEFTs != Unique_LEEFTs.end(); 
			++itUnique_LEEFTs)
	{
		//First, find all of the E6 indices.
		vector<int> E6_Indices;
		for(int a=0; a<itUnique_LEEFTs->Gauge_Groups().size(); ++a)
		{
			if((itUnique_LEEFTs->Gauge_Groups().at(a).Class() == 'E') &&
		...
		//Loop over the E6 indices, count the number of observable sector
		//charged exotics, and add them to the map.
		for(int a=0; a<E6_Indices.size(); ++a)
		{
			int E6_Index = E6_Indices.at(a);
			int Number_OS_Charged_Exotics = 
				Count_OS_Charged_Exotics(*itUnique_LEEFTs, E6_Index);
		...
		}//Close for loop over the E6 indices.
		...
		Representations, int E6_Index)
{
	...
	{
		if((a != E6_Index) && (Representations.at(a).Dimension() != 1))
			return true;
	}
	return false;
